...
import { Typography, Card, Spin, message, Image, Form, Select, Button, Input } from 'antd';
import { getFeedbackDetails, updateFeedbackDetails } from '../../../api/feedbackManagement';
...
const FeedbackDetails = () => {
  ...
  const renderRatingStars = (rating) => {
    ...
    for (let i = 1; i <= 5; i++) {
      ...
      stars.push(<span key={i} style={{ color, fontSize: '20px' }}>★</span>);
    }
    return stars;
  };
  ...
  return (
    <div style={{ padding: '20px', maxWidth: '1800px', margin: 'auto', display: 'flex', gap: '20px' }}>
      <div style={{ flex: 2 }}>
        <Title level={3} style={{ textAlign: 'center' }}>CHI TIẾT PHẢN HỒI</Title>
        <Card>
          <Form form={form} layout="vertical" onFinish={handleFinish}>
            <table style={styles.table}>
              <tbody>
                <tr className={editMode ? 'blurred-field' : ''}>
                  <td style={styles.label}><strong>ID Phản hồi:</strong></td>
                ...
                <tr className={editMode ? 'blurred-field' : ''}>
                  <td style={styles.label}><strong>Ngày gửi:</strong></td>
                  <td style={styles.value}>{new Date(data.creationDate).toLocaleString()}</td>
                </tr>
                <tr className={editMode ? 'blurred-field' : ''}>
                  <td style={styles.label}><strong>Đánh giá:</strong></td>
                ...
                <tr>
                  <td style={styles.label}><strong>Trạng thái xử lý:</strong></td>
                  <td style={styles.value}>
                  <Form.Item name="status" style={{marginTop: '15px'}}>
                    {editMode ? (
                      ...
                      <Select value={data.status} disabled style={{ width: '100%' }}>
                        <Option value={1}>Chờ xử lý</Option>
                        <Option value={2}>Đã phản hồi</Option>
                      </Select>
                    )}
      ...
      <div style={{ flex: 1, display: 'flex', flexDirection: 'column', gap: '20px' }}>
        <div style={{ flex: 1 }}>
          <Title level={4} style={{ textAlign: 'center', marginBottom: '20px' }}>THÔNG TIN CÂY</Title>
          <Card>
            <table style={styles.table}>
              <tbody>
                <tr>
                ...
                <tr>
                  <td style={styles.label}><strong>Kích thước:</strong></td>
                  <td style={styles.value}>{`${data.plant?.length} x ${data.plant?.width} x ${data.plant?.height}`}</td>
                </tr>
                <tr>
                ...
                <tr>
                  <td style={styles.label}><strong>Hình ảnh:</strong></td>
                  <td style={styles.value}>
                    <Image src={data.plant?.mainImage} alt={data.plant?.plantName} style={styles.image} />
                  </td>
                </tr>
              </tbody>
            </table>
        ...
        <div style={{ flex: 1 }}>
          <Title level={4} style={{ textAlign: 'center', marginBottom: '20px', marginTop: '20px' }}>THÔNG TIN NGƯỜI MUA</Title>
          <Card>
            <table style={styles.table}>
              <tbody>
                <tr>
                ...
                <tr>
                  <td style={styles.label}><strong>Hình ảnh:</strong></td>
                  <td style={styles.value}>
                    <img src={data.user?.imageUrl} alt={data.user?.fullName} style={styles.image} />
                  </td>
                </tr>
              </tbody>
            </table>
