...
function SideNav({ color }) {
  ...
  const dashboard = [<HomeOutlined />];

  const account = [<UserOutlined />];

  const auction = [<FolderOpenOutlined />];

  const contract = [<FileTextOutlined />];

  const product = [<FolderOpenOutlined />];

  const order = [<BellOutlined />];

  const blog = [<FileTextOutlined />];

  const feedback = [<StarOutlined />];

  const chat = [<MessageOutlined />];
    ...
    <>
      <div className="role-display">
        <Button
          type="primary"
          block
          style={{ background: "#5457FB", fontSize: "15px" }}
        >
      ...
      >
        ...
        <Menu.Item className="menu-item-header">QUẢN LÍ</Menu.Item>
